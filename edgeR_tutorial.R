...
qlf.fH.v.mH.tTags.table %>% 
  ...
  geom_text(data = filter(qlf.fH.v.mH.tTags.table, -log10(PValue) > 9), aes(label = gene)) +
  theme_minimal() +
...
fa_tpm_guts_gathered %>% 
  ...
  ggplot(aes(colour = gut, fill = gut)) +
    ...
    geom_point(data = filter(fa_tpm_guts_gathered, gene == "FBgn0032171"), aes(sample, TPM, colour = gut), position = position_dodge2(width = 0.9)) +
...
plotGene <- function(id) {
  fa_tpm_guts_gathered %>% 
    ...
    group_by(gene, sample, gut) %>% 
    summarize(mean = mean(TPM), n = n(), se = sd(TPM)/sqrt(n)) %>% 
    ggplot(aes(colour = gut)) +
    geom_col(aes(sample, mean), position = "dodge") +
    geom_errorbar(aes(x = sample, ymin = mean - se, ymax = mean + se), position = position_dodge(0.9), width = 0.3) +
    geom_point(data = filter(fa_tpm_guts_gathered, gene == id), aes(sample, TPM, colour = gut), position = position_dodge2(width = 0.9)) +
    scale_fill_brewer(palette = "Set1") -> myPlot ### must make a plot object for a function to work, then use the "return command"
  return(myPlot)
}
